...
VideoManager::setToolbox(QGCToolbox *toolbox)
{
   ...
   connect(_videoSettings->rtspUrl1(),      &Fact::rawValueChanged, this, &VideoManager::_rtspUrl1Changed);
   ...
   _updateUVC();
    ...
    for( int id = 0; id < 2; id++) {
        ...
        connect(_videoReceiver[id], &VideoReceiver::streamingChanged, this, [this](bool active){
            _streaming = active;
        ...
        connect(_videoReceiver[id], &VideoReceiver::onStopComplete, this, [this, id](VideoReceiver::STATUS) {
            _videoStarted[id] = false;
            _startReceiver(id);
        });

        connect(_videoReceiver[id], &VideoReceiver::decodingChanged, this, [this](bool active){
            _decoding = active;
            emit decodingChanged();
        });

        connect(_videoReceiver[id], &VideoReceiver::recordingChanged, this, [this](bool active){
            _recording = active;
            if (!active) {
        ...
        connect(_videoReceiver[id], &VideoReceiver::recordingStarted, this, [this, id](){
            if(id == 0)
                _subtitleWriter.startCapturingTelemetry(_videoFile);
            else
                _subtitleWriter.startCapturingTelemetry(_videoFile2);
        });

        connect(_videoReceiver[id], &VideoReceiver::videoSizeChanged, this, [this](QSize size){
...
VideoManager::startRecording(const QString& videoFile)
{
    ...
    _videoFile = savePath + "/"
            ...
            + ".";
    _videoFile2 = _videoFile + "_rtsp." + ext;
    _videoFile += ext;
    ...
    if (_videoReceiver[1] && _videoStarted[1]) {
        _videoReceiver[1]->startRecording(_videoFile2, fileFormat);
    }
...
VideoManager::grabImage(const QString& imageFile)
{
    ...
    if (imageFile.isEmpty()) {
        ...
        _imageFile2 = _imageFile + "/" + QDateTime::currentDateTime().toString("yyyy-MM-dd_hh.mm.ss.zzz") + "_rtsp.jpg";
    ...
    _videoReceiver[1]->takeScreenshot(_imageFile2);
#else
    Q_UNUSED(imageFile)
#endif
}
...
void
VideoManager::_rtspUrl1Changed()
{
    _updateVideoUri(1, _videoSettings->rtspUrl1()->rawValue().toString());
    _restartAllVideos();
}
...
VideoManager::_initVideo()
{
    ...
    for( int id = 0; id < 2; id++ ) {
        widget = root->findChild<QQuickItem*>(QStringLiteral("videoContent%1").arg(id));
        if (widget != nullptr && _videoReceiver[id] != nullptr) {
