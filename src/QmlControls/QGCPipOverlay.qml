...
Item {
    ...
    z: pipZOrder + 1
    visible: underlyingItem
             && underlyingItem.pipState !== underlyingItem.pipState.window
             && show

    property var item1: null // Required
    property var item2: null // Optional, may come and go
    property var item3: null
    property var underlyingItem: null
    property string item1IsFullSettingsKey
    // Settings key to save whether item1 was saved in full mode
    property string item2IsFullSettingsKey
    // Settings key to save whether item2 was saved in full mode
    property string item3IsFullSettingsKey
    // Settings key to save whether item3 was saved in full mode
    ...
    property var _item1IsFull
    property var _item2IsFull
    property var _item3IsFull
    ...
    function _initForItems() {
        _item1IsFull = QGroundControl.loadBoolGlobalSetting(
                    item1IsFullSettingsKey, true)
        _item2IsFull = QGroundControl.loadBoolGlobalSetting(
                    item2IsFullSettingsKey, false)
        _item3IsFull = QGroundControl.loadBoolGlobalSetting(
                    item3IsFullSettingsKey, false)
    ...
    function _swapPip() {
        if (_item1IsFull && underlyingItem === item2) {
            ...
            _item2IsFull = true
            _item3IsFull = false
        } else if (_item1IsFull && underlyingItem === item3) {
            item1.pipState.state = item1.pipState.pipState
            item2.pipState.state = item2.pipState.pipState
            item3.pipState.state = item3.pipState.fullState
            _fullItem = item3
            _pipOrWindowItem = item1
            _item1IsFull = false
            _item2IsFull = false
            ...
            item3.pipState.state = item3.pipState.pipState
            _fullItem = item1
            _pipOrWindowItem = underlyingItem
            _item1IsFull = true
            _item2IsFull = false
            _item3IsFull = false
        }

        QGroundControl.saveBoolGlobalSetting(item1IsFullSettingsKey,
                                             _item1IsFull)
        QGroundControl.saveBoolGlobalSetting(item2IsFullSettingsKey,
                                             _item2IsFull)
        QGroundControl.saveBoolGlobalSetting(item3IsFullSettingsKey,
                                             _item3IsFull)
    }
    ...
    MouseArea {
        ...
        onPressed: {
            ...
            pipResize.anchors.right
                    = undefined // This one works right, which is what we really need
        ...
        onPositionChanged: {
            if (pipResize.pressed) {
                ...
                if (newWidth < parentWidth * _maxSize
                        && newWidth > parentWidth * _minSize) {
                    _pipSize = newWidth
                }
            }
        }
    ...
    Image {
        ...
        MouseArea {
            anchors.fill: parent
            onClicked: underlyingItem.pipState.state = underlyingItem.pipState.windowState
        }
    }
    ...
    Rectangle {
        ...
        visible: !_isExpanded
        color: _fullItem.pipState.isDark ? Qt.rgba(0, 0, 0,
                                                   0.75) : Qt.rgba(0, 0, 0, 0.5)
        Image {
